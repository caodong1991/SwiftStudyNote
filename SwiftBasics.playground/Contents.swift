...
// 类型标注
// 当声明变量或常量的时候可以加上类型标注（type annotation）, 说明常量或变量中药存储的值的类型。如果要添加类型标注，需要在常量或者变量名后面加上冒号和空格，然后加上类型名称。
...
// 用字符串插值的方式把常量名或者变量名当做占位符加入到长字符串中，Swift会用当前常量或变量的值替换这些占位符。将常量或变量名放入圆括号中，并在开括号前使用反斜杠将其转义
