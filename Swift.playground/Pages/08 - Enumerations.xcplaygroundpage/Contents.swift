...
/*
 ...
 与OC不同，枚举成员在被创建的时候不会被赋予一个默认的整型值。相反，这些枚举成员本身就是完备的值，这些值的类型是已经明确定义好的CompassPoint类型。
 */
enum CompassPoint {
    case north
    case south
...
/*
 每个枚举定义了一个全新的类型。枚举名以一个大写字母开头。给枚举类型起一个单数名字而不是复数名字。
...
// 关联值
/*
 为枚举成员设置一个常量或变量，并且在赋值之后查看这个值。
 把其他类型的值和成员值一起存储起来会很有用，这额外的信息被称为关联值。
 使用该枚举成员时，可以修改这个关联值。
